...
with Reader(bag_path) as reader:
    ...
    for connection, timestamp, rawdata in reader.messages():
        ...
        if connection.topic == '/oculus/raw_data':
            ...
            img_cart = polar_to_cart(polar_image_data).cart_image
            img = polar_image_data.polar_image
            gain = polar_image_data.gain_table
            range = polar_image_data.ranging_table
            bearing = polar_image_data.bearing_table

            # Display the image
            cv2.imshow("backscatter", img)
            cv2.imshow("cartesian", img_cart)

            key = cv2.waitKey(10)
            if key & 0xFF == ord('s'):
                # Save img, gain, range and bearing as numpy arrays
                np.save('samples/polar/img.npy', img)
                np.save('samples/polar/gain.npy', gain)
                np.save('samples/polar/range.npy', range)
                np.save('samples/polar/bearing.npy', bearing)
            if key & 0xFF == ord('q'):
                break
